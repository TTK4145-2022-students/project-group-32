...
func ComputeETA(
	aboveOrAtFloor int, destinationFloor int) time.Time {
	...
	newETA = newETA.Add(time.Second * timeEstimatePerOrder * time.Duration(orderstate.OrdersBetween(aboveOrAtFloor, destinationFloor)))
	return newETA
}
